...
What I'll need to eventually implement, though, is dynamic type creation (read a json file, establish the what function must be called and in which sequence, if any, add the new specified components, if any, modify all the necessary components as needed, all dynamically figured out). 
