...
namespace Test
{
    public class BlockingTrap : Entity
    {
        ...
        public static EntityFactory<Barrier> BarrierFactory =
            Barrier.CreateFactory()
                .AddBehavior<Attackable>()
                .AddBehavior<Damageable>();

        private static Layer TargetedLayer = Layer.REAL;
        [DataMember] private List<Barrier> m_barriers;
        ...
        {
            if (m_barriers != null && Cell.GetEntityFromLayer(TargetedLayer) == null)
            {
                KillBarriers();
            }
        }
